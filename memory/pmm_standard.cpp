...
{
    bitmap_pointer = reinterpret_cast<ulong*>(0x8000);
    pmm_max_blocks = (memory_size * 1000) / BLOCK_SIZE;
    pmm_used_blocks = pmm_max_blocks;
    bitmap_size = pmm_max_blocks / (BLOCKS_PER_BYTE * sizeof(ulong));
    System::GetInstance() << info << "PMM max blocks: " << pmm_max_blocks << video::endl
                          << info << "PMM bitmap size: " << bitmap_size << video::endl;
    ...
    while(reinterpret_cast<ulong>(mmap) < multiboot_info->mmap_addr + multiboot_info->mmap_length)
    {
...
void* StandardPhysicalMemoryManager::AllocateBlocks(size_t number_of_blocks)
{
...
void StandardPhysicalMemoryManager::FreeBlocks(void* block_address, size_t number_of_blocks)
{
    ulong temp_address = reinterpret_cast<ulong>(block_address);
    int frame = temp_address / BLOCK_SIZE;
...
uint StandardPhysicalMemoryManager::IsRegionUsed(ulong base_address, size_t memory_size)
{
    uint result = 0;
    size_t free_bit_counter = 0;
    for(uint block_number = memory_size / BLOCK_SIZE, block_bit = base_address / BLOCK_SIZE; block_number > 0; block_number--, block_bit++, pmm_used_blocks--)
    {
    ...
    if((free_bit_counter * BLOCK_SIZE) == memory_size)
        result = PMM_REGION_NOT_USED;
    else if((free_bit_counter * BLOCK_SIZE) < memory_size)
        result = PMM_REGION_PARTLY_USED;
    else
        result = PMM_REGION_FULLY_USED;
    return result;
}

void StandardPhysicalMemoryManager::InitRegion(ulong base_address, size_t memory_size)
{
    for(uint blocks = memory_size / BLOCK_SIZE, align = base_address / BLOCK_SIZE; blocks > 0; blocks--, align++, pmm_used_blocks--)
        ClearBit(align);

    SetBit(0);	//first block is always set. This insures allocs cant be 0
}

void StandardPhysicalMemoryManager::DeinitRegion(ulong base_address, size_t memory_size)
{
    for(uint blocks = memory_size / BLOCK_SIZE, align = base_address / BLOCK_SIZE; blocks > 0; blocks--, align++, pmm_used_blocks++)
        SetBit(align);
